...
var filename = 'assets/GaltonFamilies.csv'
var colX = 'father'     // X-aksen: Variabel 1 (input)
var colY = 'mother'      // Y-aksen: Variabel 2 (input)
var colLabel = 'childHeight' // Facit: Hvilken gruppe hører man til?

// GUI Overskrifter (Gør det pænt for brugeren)
var mainTitle = "Child Height Predictor"
var sectionTitle1 = "1. Indtast dine tal"
var instructionText = "Angiv højde for far og mor i inches:"
...
function setup() {
    ...
    //Vi skal have fat i de unikke labes for hver gruppe i data
    var uniqueLabels = []
    data.map(point => {
    ...
    var datasets = uniqueLabels.map((label, index) => {
        ...
        //returner den FÆRDIGE grupppe med alle datapunkterne for hvert label til datasæt
        return {
            label: label,
            data: groupData,
            backgroundColor: col,
    ...
    console.log('Så fik vi lavet datasæt grupperne')
...
function setupControls() {
    ...
    //2) Fordi vi skal bruge dem til at bestemme hvad de der sliderer skal gå fra og til
    ...
    console.log('Her er mine min og max værdier for x og y', minX, maxX, minY, maxY)
...
function classifyUnknown(){
    ...
    //Indsæt punktet fra slidernei grafen
    ...
    //Object.keys giver os navnene på nøglerne, i dette tilfælde er det jo vores labels
    var allLabels = Object.keys(votes)

    //Start med bare at sige at vinderen er den første label
    var winner = allLabels[0]

//Løb alle labels igennem og se hvem der så virkelig er vinderen
allLabels.map( l => {
    if(votes[l] > votes[winner]){
        winner = l
    }
})
    //Vis i resultat feltet hvilken klasse gættet tilhører
